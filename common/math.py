...
def two_hot(x, cfg):
	...
	# --- safety checks (avoid CUDA scatter crash) ---
	if cfg.bin_size is None or float(cfg.bin_size) == 0.0:
		raise RuntimeError(f"cfg.bin_size is invalid: {cfg.bin_size}")

	if (bin_idx < 0).any() or (bin_idx >= cfg.num_bins).any():
		raise RuntimeError(
			f"bin_idx out of range! "
			f"min={bin_idx.min().item()}, max={bin_idx.max().item()}, num_bins={cfg.num_bins}, "
			f"vmin={cfg.vmin}, vmax={cfg.vmax}, bin_size={cfg.bin_size}"
		)
