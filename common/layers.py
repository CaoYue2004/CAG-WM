...
from .tcnencoder import DictObsEncoderTCN
...
class Ensemble(nn.Module):
    """
    Vectorized ensemble WITHOUT tensordict.from_modules.
    Uses torch.func.stack_module_state + functional_call + vmap.
    """
    def __init__(self, modules, **kwargs):
        ...
        for k, v in buffers.items():
            self.register_buffer(f"_b__{self._bmap[k]}", v, persistent=False)

    @property
    def params(self):
        return _ParamView({k: getattr(self, f"_p__{self._pmap[k]}") for k in self._param_keys})

    @property
    def buffers(self):
        return _ParamView({k: getattr(self, f"_b__{self._bmap[k]}") for k in self._buffer_keys})
    ...
    def forward(self, *args, params=None, buffers=None, **kwargs):
        params = self.params if params is None else params
        buffers = self.buffers if buffers is None else buffers

        params = self._as_pytree_dict(params)
        buffers = self._as_pytree_dict(buffers)

        return torch.vmap(self._call, in_dims=(0, 0, None), randomness="different")(
