...
# for no up 512 x 512 generation
@persistence.persistent_class
class SuperresolutionNoUp512(torch.nn.Module):
    def __init__(self, channels, img_resolution, sr_num_fp16_res, sr_antialias,
                 num_fp16_res=4, conv_clamp=None, channel_base=None, channel_max=None,  # IGNORE
                 **block_kwargs):
        super().__init__()
        ...
        self.sr_antialias = sr_antialias
        self.block0 = SynthesisBlockNoUp(channels, 128, w_dim=512, resolution=512,
                                         img_channels=3, is_last=False, use_fp16=use_fp16,
                                         conv_clamp=(256 if use_fp16 else None), **block_kwargs)
        self.block1 = SynthesisBlockNoUp(128, 64, w_dim=512, resolution=512,
                                         img_channels=3, is_last=False, use_fp16=use_fp16,
                                         conv_clamp=(256 if use_fp16 else None), **block_kwargs)
        self.register_buffer('resample_filter', upfirdn2d.setup_filter([1, 3, 3, 1]))
